...
class BajajFinservChatbot:
        ...
        try:
            ...
            if len(periods) >= 2:
                ...
                return f"📊 **Bajaj Finserv Comparison: {period1[0]}-{period1[1]} vs {period2[0]}-{period2[1]}**\n\n" \
                       ...
                       f"• Average Price: ₹{metrics1['avg_price']:.2f}\n" \
                       f"• Highest Price: ₹{metrics1['high_price']:.2f}\n" \
                       f"• Lowest Price: ₹{metrics1['low_price']:.2f}\n" \
                       f"• Volatility: {metrics1['volatility']:.2f}%\n\n" \
                       f"**Period 2 ({period2[0]}-{period2[1]}):**\n" \
                       f"• Average Price: ₹{metrics2['avg_price']:.2f}\n" \
                       f"• Highest Price: ₹{metrics2['high_price']:.2f}\n" \
                       f"• Lowest Price: ₹{metrics2['low_price']:.2f}\n" \
    ...
    def handle_allianz_stake_questions(self, question):
        ...
        try:
            ...
            response = "🏛️ **Allianz Stake Sale Discussions:**\n\n"
            
            # Create a table format for dates and discussions
            response += "| Date | Quarter | Key Discussion Points |\n"
            response += "|------|---------|----------------------|\n"
            
            for quarter, info in relevant_info.items():
                # Extract key dates and points
                key_points = info[:200] + "..." if len(info) > 200 else info
                response += f"| {quarter} | {quarter} | {key_points} |\n"
    ...
    def handle_cfo_commentary_questions(self, question):
        ...
        try:
            ...
            response += f"• Our stock has shown resilience with current levels at ₹{latest_data['current_price']:.2f}\n"
...
with gr.Blocks(title="🏦 Bajaj Finserv AI Chatbot", theme=gr.themes.Soft()) as demo:
    ...
    - 🏛️ **Allianz Stake Sale**: Discussions and timelines about stake divestment
